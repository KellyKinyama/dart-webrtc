...
Uint8List constructStunBindingRequest(Uint8List transactionId) {
  ...
  // Add message type (big-endian)
  buffer.addByte((messageType >> 8) & 0xFF);
  buffer.addByte(messageType & 0xFF);

  // Add message length (big-endian)
  buffer.addByte((messageLength >> 8) & 0xFF);
  buffer.addByte(messageLength & 0xFF);
...
void parseStunBindingResponse(Uint8List data, Uint8List originalTransactionId) {
  ...
  if (messageType == 0x0101) {
    ...
    while (attributeOffset < data.length) {
      ...
      switch (attributeType) {
        ...
        default:
          print(
              'Received unknown attribute: 0x${attributeType.toRadixString(16).padLeft(4, '0')} (Length: $attributeLength)');
      }
  ...
  } else if (messageType == 0x0111) {
    // Binding Error Response
    print('Received STUN Binding Response (Error)');
    // Implement error code parsing here
  } else {
    print(
        'Received unknown STUN message type: 0x${messageType.toRadixString(16).padLeft(4, '0')}');
  }
}
...
void parseXorMappedAddressAttribute(Uint8List attributeValue) {
  ...
  if (family == 0x01) {
    ...
    print(
        'Discovered public IPv4 address: $address:$port (via XOR-MAPPED-ADDRESS)');
  } else if (family == 0x02) {
    // IPv6
...
Future<void> main() async {
  ...
  try {
    ...
    final List<InternetAddress> addresses =
        await InternetAddress.lookup(stunServerAddress);
    if (addresses.isEmpty) {
      print('Failed to resolve host: $stunServerAddress');
      return;
    }
    final remoteAddress = InternetAddress(addresses.first.address);
