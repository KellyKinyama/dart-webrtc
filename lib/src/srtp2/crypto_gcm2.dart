...
class GCM {
  ...
  static Future<GCM> newGCM(Uint8List masterKey, Uint8List masterSalt) async {
    ...
    final srtpBlockCipher = GCMBlockCipher(AESEngine());
    ...
    final srtcpBlockCipher = GCMBlockCipher(AESEngine());

    // AEADParameters paramsEnc =
    //     AEADParameters(KeyParameter(srtcpSessionKey), 32, nonce, aad);
  ...
  Future<Uint8List> decrypt(Packet packet, int roc) async {
    ...
    final ParametersWithIV<AEADParameters> params = ParametersWithIV(
      AEADParameters(
          KeyParameter(Uint8List(0)), aeadAuthTagLen * 8, iv, headerBytes),
      iv,
    );
