import 'dart:typed_data';
import 'dart:convert';

import '../ecdsa3.dart';
import '../key_exchange_algorithm.dart';

const RAW_PRIVATE_KEY = """
-----BEGIN RSA PRIVATE KEY-----

JPhfPySIPG4UmwE4gW8t79vfOKxnUu2fDD1ZXUYopan6EckACNH/
-----END RSA PRIVATE KEY-----
""";

void testGenerateKeySignature() {
  final privateKey = decodePemToDer(RAW_PRIVATE_KEY);
  final signatureBytes = generateKeySignature(
      clientRandom, serverRandom, publicKey, decodePemToDer(RAW_PRIVATE_KEY));

  // print("Got signature:      ${signatureBytes.length}");

  final msg = generateKeyValueMessages(
      clientRandom, serverRandom, publicKey, privateKey);

  ecdsaVerify(publicKey, msg, signatureBytes);
  print("Verify signature: ${expected_signature.length}");
}

// Function to decode a PEM string to DER bytes
Uint8List decodePemToDer(String pemString) {
  // Remove the header and footer lines
  final String base64String = pemString
      .replaceAll('-----BEGIN CERTIFICATE-----', '')
      .replaceAll('-----END CERTIFICATE-----', '')
      .replaceAll('-----BEGIN RSA PRIVATE KEY-----', '')
      .replaceAll('-----END RSA PRIVATE KEY-----', '')
      .replaceAll('\n', '')
      .replaceAll('\r', '')
      .trim();

  // Decode the Base64 string to bytes (which are the DER bytes)
  return base64Decode(base64String);
}

void main() {
  // final derBytes = decodePemToDer(RAW_PRIVATE_KEY);
  testGenerateKeySignature();
  // print("Decoded Private Key in DER format: $derBytes");
}

final clientRandom = Uint8List.fromList([
  0x00,
  0x01,
  0x02,
  0x03,
  0x04,
  0x05,
  0x06,
  0x07,
  0x08,
  0x09,
  0x0a,
  0x0b,
  0x0c,
  0x0d,
  0x0e,
  0x0f,
  0x10,
  0x11,
  0x12,
  0x13,
  0x14,
  0x15,
  0x16,
  0x17,
  0x18,
  0x19,
  0x1a,
  0x1b,
  0x1c,
  0x1d,
  0x1e,
  0x1f,
]);
final serverRandom = Uint8List.fromList([
  0x70,
  0x71,
  0x72,
  0x73,
  0x74,
  0x75,
  0x76,
  0x77,
  0x78,
  0x79,
  0x7a,
  0x7b,
  0x7c,
  0x7d,
  0x7e,
  0x7f,
  0x80,
  0x81,
  0x82,
  0x83,
  0x84,
  0x85,
  0x86,
  0x87,
  0x88,
  0x89,
  0x8a,
  0x8b,
  0x8c,
  0x8d,
  0x8e,
  0x8f,
]);
final publicKey = Uint8List.fromList([
  0x20,
  0x9f,
  0xd7,
  0xad,
  0x6d,
  0xcf,
  0xf4,
  0x29,
  0x8d,
  0xd3,
  0xf9,
  0x6d,
  0x5b,
  0x1b,
  0x2a,
  0xf9,
  0x10,
  0xa0,
  0x53,
  0x5b,
  0x14,
  0x88,
  0xd7,
  0xf8,
  0xfa,
  0xbb,
  0x34,
  0x9a,
  0x98,
  0x28,
  0x80,
  0xb6,
  0x15,
]);
final expected_signature = Uint8List.fromList([
  ,
]);
