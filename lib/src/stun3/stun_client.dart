...
class StunClient {
  ...
  Future<void> sendBindingRequest() async {
    ...
    print(
        'STUN Client bound to local address: ${_socket!.address.host}:${_socket!.port}');

    Uint8List transactionId = _generateTransactionId();

    // Construct the USERNAME attribute as "serverUfrag:clientUfrag"
    // Note: For public STUN servers like Google's, USERNAME and PASSWORD
    // are often not required for a basic binding request.
    // If a server requires it, the username format is typically:
    // remote-ufrag:local-ufrag or local-ufrag:remote-ufrag based on role.
    StunAttribute? usernameAttr;
    if (clientUfrag.isNotEmpty && serverUfrag.isNotEmpty) {
      String usernameString =
          '$serverUfrag:$clientUfrag'; // Or clientUfrag:serverUfrag depending on ICE role
    ...
    // Encode the message. For public STUN servers, the password might be null
    // as MESSAGE-INTEGRITY is not always required.
    ...
    try {
      // Decode the encoded request for logging purposes to see what's actually sent
      StunMessage decodedSentRequest = StunMessage.decode(encodedRequest);
      print('Encoded STUN Request (for verification):');
      print(decodedSentRequest);
    } catch (e) {
      print("Could not decode the sent request for logging: $e");
    }

    _socket!.send(encodedRequest, _serverHost, _serverPort);
    ...
    _socket!.listen((RawSocketEvent event) {
      if (event == RawSocketEvent.read) {
        ...
        if (datagram != null) {
          _handleResponse(datagram, transactionId);
          _socket?.close(); // Close socket after receiving response
        }
      }
    });

    // Optional: Add a timeout for the response
    // Future.delayed(Duration(seconds: 5), () {
    //   if (_socket != null && _socket!.isActive) {
    //     print('STUN response timed out.');
    //     _socket?.close();
    //   }
    // });
  }

  void _handleResponse(Datagram datagram, Uint8List sentTransactionId) {
    print(
    ...
    try {
      ...
      // Validate the response (only if password is provided for integrity check)
      bool isValid = response.validate(
          passwordForIntegrity:
              serverPassword.isNotEmpty ? serverPassword : null);
      if (!isValid) {
        print("STUN Response validation failed.");
        // Continue processing even if validation fails, as some public servers might not use integrity.
      }
      ...
      if (response.messageType == StunMessageType.bindingSuccessResponse) {
        ...
        if (xorMappedAddressAttr != null) {
          ...
          print('Your external IP: ${mappedAddress.ip.address}');
          print('Your external Port: ${mappedAddress.port}');
        } else {
          print('XOR-MAPPED-ADDRESS attribute not found in response.');
        }
      } else if (response.messageType == StunMessageType.bindingErrorResponse) {
        print('STUN Binding Error Response received!');
        ...
        print('Received unexpected STUN message type: ${response.messageType}');
      }
    } catch (e, s) {
      print('Error processing STUN response: $e');
      print('Stack trace: $s');
    }
  }
...
Future<void> main(List<String> arguments) async {
  // Default server address and port for testing
  final serverHost =
      InternetAddress.loopbackIPv4; // Use 127.0.0.1 for local testing
  final serverPort = 4444; // Must match the server's listening port

  // These credentials should match what the STUN server expects
  // For this example, matching the defaults in stun_server.dart
  String clientUfrag = "client-ufrag-from-clients-sdp";
  String serverUfrag = "yxYb";
  String serverPassword = "05iMxO9GujD2fUWXSoi0ByNd";

  // if (arguments.length > 0) serverHost.host = arguments[0];
  // if (arguments.length > 1) serverPort = int.tryParse(arguments[1]) ?? 4444;
  ...
  print('  Server: ${serverHost.host}:${serverPort}');
  print('  Client Ufrag: $clientUfrag');
  print('  Server Ufrag: $serverUfrag');
  print('  Server Password: $serverPassword (for integrity check)');
