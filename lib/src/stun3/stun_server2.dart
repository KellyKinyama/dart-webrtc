...
Future<void> startStunServer(int port) async {
  ...
  await for (RawSocketEvent event in socket) {
    if (event == RawSocketEvent.read) {
      ...
      if (datagram != null) {
        print('Received packet from ${datagram.data}');
  ...
  while (offset < requestBytes.length) {
    ...
    switch (attrType) {
      case stunAttrMappedAddress: // 0x0001
        print(
            '  Received MAPPED-ADDRESS attribute (0x0001). Length: $attrLength, Value: $attrValue');
        // You could parse the MAPPED-ADDRESS here if needed.
        // For IPv4: [0x00, 0x01, Port (2 bytes), IP (4 bytes)]
        if (attrLength >= 8) {
          final int family = attrValue[1];
          final int port = ByteData.view(attrValue.buffer).getUint16(2);
          String ip = '';
          if (family == 0x01 && attrLength >= 8) {
            ...
            print(
                '    MAPPED-ADDRESS: Unknown family ($family) or invalid length for family.');
          }
        } else {
          print('    MAPPED-ADDRESS value too short.');
        }
        break;
